...
<script>
    ...
    <c:forEach items="${list}" var="list">
       <ul>
           <li>
               <div>
                   <div>${list.bno}</div>
                   <div><a href="/board/read?bno=${list.bno}&page=${pageMaker.page}&size=${pageMaker.size}">${list.title}</a></div>
    ...
    <ul class="page">
        <c:if test="${pageMaker.prev}">
            <li>
                <a href="/board/list?page=${pageMaker.start - 1}">PREV</a>
            </li>
        </c:if>
    <c:forEach begin="${pageMaker.start}" end="${pageMaker.end}" var="page">
        ...
        <c:if test="${pageMaker.next}">
            <li>
                <a href="/board/list?page=${pageMaker.end + 1}">NEXT</a>
            </li>
        </c:if>
    </ul>
